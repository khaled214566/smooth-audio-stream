...
export const AudioProvider = ({ children }: { children: ReactNode }) => {
  ...
  useEffect(() => {
    ...
    const unsubscribe = libraryService.subscribe((librarySongs) => {
      // Load favorites from storage and apply to songs
      const favoriteIds = StorageService.getFavorites();
      const songsWithFavorites = librarySongs.map(song => ({
        ...song,
  ...
  const toggleFavorite = useCallback((songId: string) => {
    if (StorageService.isFavorite(songId)) {
      StorageService.removeFavorite(songId);
    } else {
      StorageService.addFavorite(songId);
    }
    
    // Update the song in the library to reflect favorite status
    setSongs(prevSongs => 
      prevSongs.map(song => 
