...
function downloadAudio(url, format, quality, downloadId, videoTitle) {
  return new Promise((resolve, reject) => {
    const audioFormat = format === 'wav' ? 'wav' : 'mp3';
    const qualityBitrate = quality + 'k';

    const safeName = sanitizeFilename(videoTitle || `download_${downloadId}`);
    const filename = `${safeName}.${audioFormat}`;
    ...
    const ytDlp = spawn('yt-dlp', [
      ...
      '--embed-thumbnail',
      '--embed-metadata',
      '--newline',                   // Force one progress line per output (key for parsing)
      '-o', outputPath,
      url
    ]);
    ...
    // yt-dlp also writes progress to stderr in some versions — capture both
    ytDlp.stderr.on('data', (chunk) => {
    ...
    ytDlp.on('close', (code) => {
      if (code === 0) {
        // Find the actual output file — yt-dlp may adjust the extension
        const files = fs.readdirSync(audioDir).filter(f => f.startsWith(safeName));
        if (files.length > 0) {
  ...
  if (req.url === '/api/download' && req.method === 'POST') {
    ...
    req.on('end', async () => {
      try {
        ...
        // Start download in background — pass the video title for naming
        downloadAudio(url, format, quality, downloadId, videoInfo.title)
          .then((result) => {
